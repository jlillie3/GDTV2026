...
static func add_temporary_effect_to(effect_scene, in_time: float, node) -> Effect:
	var effect = effect_scene.new()
	return add_premade_temporary_effect_to(effect, in_time, node)
	
static func add_effect(effect_scene, node) -> Effect:
	var effect
	if effect_scene is PackedScene:
		effect = effect_scene.instantiate()
	else:
		effect = effect_scene.new()
...
static func add_temporary_multi_effect_to(effect_scenes: Array, in_times: Array[float], node) -> void:
	var detachable = Detachable.new()
	var serial_effect = preload("res://toolkit/nodes/effects/serial_effect.gd").new()
	var total_time = 0.0
...
static func add_temporary_parallel_multi_effect_to(effect_scenes: Array, in_time, node) -> void:
	var detachable = Detachable.new()
	var parallel_effect = preload("res://toolkit/nodes/effects/parallel_effect.gd").new()
	
	for i in effect_scenes.size():
		parallel_effect.add_effect(effect_scenes[i].new())
...
static func shake(canvas_item: CanvasItem, amount: float) -> void:
	var shake_effect = preload("res://toolkit/nodes/effects/shake.gd").new()
